...
async def main():
    ...
    async with httpx.AsyncClient() as client:
        ...
        url_for_docker = data['url'].replace('localhost:9000', 'minio:9000')
        # download from URL
        resp2 = await client.get(url_for_docker)
        print('Downloaded Content (first 100 bytes):', repr(resp2.content[:100]))
