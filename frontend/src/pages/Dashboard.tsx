...
function computeHealthScore(params: {
  ...
  let score = 100;
  if (!params.cycleStatus || params.cycleStatus === "DRAFT") score -= 20;
  if (params.failedResults > 0)
    score -= Math.min(40, params.failedResults * 8);
  if (params.openIssues > 0)
    score -= Math.min(20, params.openIssues * 5);
  if (!params.hasCompliance) score -= 15;
...
function scoreBg(score: number): string {
  ...
  return "bg-danger-light dark:bg-danger/10";
}
...
function KpiCard({
  ...
  const inner = (
    ...
    >
      ...
      {to && (
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none rounded-[inherit]"
          style={{ background: "radial-gradient(circle at top left, rgba(99,102,241,0.04), transparent 70%)" }} />
      )}
      <div className="flex items-start justify-between">
        <div className={clsx("flex h-9 w-9 items-center justify-center rounded-xl", iconBg)}>
          <Icon
            className={clsx("shrink-0", danger ? "text-danger" : iconColor)}
            style={{ width: 18, height: 18 }}
          />
        </div>
        {to && (
          <ChevronRight className="h-3.5 w-3.5 text-slate-300 group-hover:text-slate-500 dark:text-slate-700 dark:group-hover:text-slate-400 transition-colors" />
        )}
        {locked && (
      ...
      <div>
        ...
        <div className={clsx("kpi-value mt-1 font-display", danger && "text-danger")}>
          {loading ? (
            <Skeleton className="h-7 w-28" />
          ) : locked ? (
            <span className="text-slate-300 dark:text-slate-700 text-lg select-none">••••••</span>
          ) : (
            (value ?? "—")
          )}
        </div>
...
const CustomTooltip = memo(function CustomTooltip({
  ...
  return (
    <div className="glass-panel px-4 py-3 min-w-[160px]">
      ...
      {payload.map((p: any) => (
        <div key={p.name} className="flex items-center gap-2 text-sm">
          ...
          <span className="font-semibold text-slate-900 dark:text-slate-100 font-numeric tabular-nums">
            {formatINR(p.value)}
          </span>
        </div>
      ))}
...
export function Dashboard() {
  ...
  const trendSummaries = useQuery({
    ...
    queryFn: async () =>
      Promise.all(trendIds.map((id) => payrollApi.getCycleSummary(id))),
  ...
  const totalLOP = useMemo(
    () =>
      latestResults.reduce((s, r) => {
        const lop = parseFloat(
          r.breakdown_json?.attendance?.lop_days ?? "0"
        );
        return s + lop;
      }, 0),
    [latestResults]
  ...
  const headcountByDept = useMemo(() => {
    ...
    allEmpQ.data.items.forEach((e) => {
      const dname = e.department_id
        ? (deptMap[e.department_id] ?? "Unknown")
        : "No Dept";
  ...
  const healthScore = useMemo(
    () =>
      computeHealthScore({
        ...
        openIssues,
        hasCompliance: !!complianceQ.data,
      }),
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 xl:grid-cols-6">
        ...
        <KpiCard
          ...
          value={
            <span
              className={clsx("font-display tabular-nums", scoreColor(healthScore))}
            >
              {healthScore}
              <span className="text-sm font-medium ml-0.5 opacity-70">/100</span>
            </span>
          }
          sub={
            healthScore >= 80
          ...
          loading={cyclesQ.isLoading || summaryQ.isLoading}
          icon={Heart}
          iconColor={scoreColor(healthScore)}
          iconBg={scoreBg(healthScore)}
        />
      </div>
          ...
          {trendSummaries.isLoading || cyclesQ.isLoading ? (
            ...
            <ResponsiveContainer width="100%" height={220}>
              ...
              >
                ...
                <Legend
                  formatter={(v) => (
                    <span style={{ fontSize: 11, color: "var(--chart-axis)" }}>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <div className="mb-4 flex items-center justify-between">
            ...
            {latestCycle && (
              <Link
                ...
                className="flex items-center gap-1 text-xs font-semibold text-accent-600 hover:text-accent-700 dark:text-accent-400 transition-colors"
              >
          ...
          {complianceQ.data &&
            (() => {
              ...
              const items = [
                {
                  label: "PF (Employee)",
                  value: parseFloat(t.total_employee_pf),
                  color: "bg-accent-500",
                },
                {
                  label: "PF (Employer)",
                ...
                {
                  label: "ESI Employee",
                  value: parseFloat(t.total_employee_esi),
                  color: "bg-emerald-500",
                },
                {
                  label: "ESI Employer",
                  value: parseFloat(t.total_employer_esi),
                  color: "bg-teal-500",
                },
                {
                  label: "Prof. Tax",
                  value: parseFloat(t.total_pt),
                  color: "bg-amber-500",
                },
              ];
              ...
              return (
                <div className="space-y-3">
                  {items.map((item) => (
                    <div key={item.label}>
                      <div className="flex items-center justify-between text-xs mb-1">
                        ...
                        <span className="font-numeric font-semibold text-slate-800 dark:text-slate-200 tabular-nums">
                  ...
                  <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">
                    {t.ceiling_applied_count} with PF ceiling ·{" "}
                    {t.esi_eligible_count} ESI eligible
                  </p>
                </div>
              );
            })()}
        ...
        <div className="card">
                ...
                <div className="flex flex-col items-center">
                  <span
                    className={clsx(
                      ...
                      EVENT_COLORS[e.event_type] ?? "bg-slate-300"
                    )}
                  />
                  {i < (auditQ.data?.slice(0, 8).length ?? 0) - 1 && (
                    <div
                      className="w-px flex-1 bg-slate-100 dark:bg-slate-800 mt-1"
                      style={{ minHeight: 14 }}
                    />
                  )}
                </div>
          ...
          <Link
            to="/audit"
            className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-accent-600 hover:text-accent-700 dark:text-accent-400 transition-colors"
          >
        ...
        <div className="card">
          ...
          <div className="space-y-2">
            {deadlines.map((d) => (
              <div
                key={d.id}
                className="flex items-center justify-between rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/25 px-4 py-3"
              >
                <div>
                ...
                <div
                  className={clsx(
                    "shrink-0 rounded-lg px-3 py-1.5 text-xs font-bold tabular-nums",
                    d.daysLeft <= 3
...
function ActionItems({
  ...
  if (!hasIssues) {
    return (
      <div className="flex items-center gap-3 rounded-xl bg-emerald-50 dark:bg-emerald-900/15 border border-emerald-100 dark:border-emerald-900/25 px-4 py-3">
    ...
    <div className="space-y-2">
      {failedCycles.map((c) => (
        <Link
          key={c.id}
          to={`/cycles/${c.id}`}
          className="flex items-center justify-between rounded-xl border border-danger/20 bg-danger-light dark:bg-danger/8 px-4 py-3 hover:bg-danger/10 dark:hover:bg-danger/15 transition-colors"
        >
          <div className="text-sm">
            <span className="font-semibold text-danger-dark dark:text-danger">
              Cycle FAILED:{" "}
            </span>
      ...
      {failedResults.length > 0 && (
        <div className="rounded-xl border border-danger/20 bg-danger-light dark:bg-danger/8 px-4 py-3">
          <div className="text-sm font-semibold text-danger-dark dark:text-danger">
            {failedResults.length} payroll result
            {failedResults.length !== 1 ? "s" : ""} failed
          </div>
          <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            Review in Pay Run Summary
          </div>
        </div>
      )}
    </div>
